...
public interface TransactionRepository extends JpaRepository<Transaction, BigInteger> {
        @Query(value = "select t.transaction_time from transaction t join request r on r.id=t.request_id where r.id=:requestId order by t.transaction_time desc limit 1", nativeQuery = true)
        Optional<Date> getLastTransactionDate(BigInteger requestId);
